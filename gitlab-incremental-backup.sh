...
existed_copies=`ls | grep "gitlab_backup.tar" | wc -l`
...
if [[ $existed_copies == 0 ]]; then
    ...
    # touch "$(date +%s)_gitlab_backup.tar"
    ...
    if [ $? -ne 0 ]; then
        ...
        new_backup=`ls *.tar -t | head -n1`
        echo "$(date '+%Y-%m-%d, %H:%M:%S %Z') -- 备份成功：${new_backup}" >> $LOG_FILE
    fi
else 
    previous_backup=`ls *.tar -t | head -n1`
